 ...
 // Floating background subtle motion
	...
	async function loadModalContent(modalKey) {
		...
		try {
			...
			if (!response.ok) return fallback[modalKey] || null;
			const data = await response.json();
			return data[modalKey] || fallback[modalKey] || null;
		} catch (err) {
			console.warn('API indisponible, utilisation du fallback:', err);
			return fallback[modalKey] || null;
		}
	}
